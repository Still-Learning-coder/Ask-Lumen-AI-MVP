...
serve(async (req) => {
  ...
  socket.onopen = async () => {
    ...
    try {
      // Create WebSocket connection to AssemblyAI v3 streaming API with opus encoding
      const url = `wss://streaming.assemblyai.com/v3/ws?sample_rate=16000&encoding=opus&token=${ASSEMBLYAI_API_KEY}`;
